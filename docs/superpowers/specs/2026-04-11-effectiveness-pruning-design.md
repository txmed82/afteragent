...
| Single replay row has malformed `comparison_json` or `intervention_manifest_json` | Aggregator's per-row try/except skips the row and continues. Not logged per-row (noisy); total sample counts reflect the skipped row via a shortage in `total_replays`. |
